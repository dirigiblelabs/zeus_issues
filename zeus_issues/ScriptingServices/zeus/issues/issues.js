...
var issuesDao = require('zeus/issues/issuesDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			var entity = issuesDao.get(id);
			if (entity !== null) {
				sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(entity, null, 2));
			} else {
				sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'issue_id\'=' + id);
			}
		} else if (count !== null) {
			var issuesCount = issuesDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', issuesCount);
		} else if (metadata !== null) {
			var issuesMetadata = issuesDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(issuesMetadata, null, 2));
		} else {
			var issues = issuesDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(issues, null, 2));
		}
	}
}
...
function handlePostRequest(httpRequest, httpResponse) {
	...
	var id = issuesDao.create(entity);
...
function handlePutRequest(httpRequest, httpResponse) {
	var entity = getRequestBody(httpRequest);
	var id = getIdParameter(httpRequest, xss);
	id = id !== null ? id : entity.issue_id;
	if (id !== null) {
		if (issuesDao.get(id) !== null) {
			issuesDao.update(entity);
			sendResponse(httpResponse, httpResponse.NO_CONTENT);
		} else {
			sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'issue_id\'=' + id);
		}
	} else {
		sendResponse(httpResponse, httpResponse.PRECONDITION_FAILED, 'text/plain', 'Expected \'issue_id\' parameter is missing!');
	}
...
function getIdParameter(httpRequest, xss) {
	...
	id = id !== null ? id : xss.escapeSql(httpRequest.getParameter('issue_id'));
	return id;
}
