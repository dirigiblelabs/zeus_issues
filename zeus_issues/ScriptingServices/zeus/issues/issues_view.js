...
var issues_viewDao = require('zeus/issues/issues_viewDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(null, count, metadata, httpResponse)) {
		if (count !== null) {
			var issues_viewCount = issues_viewDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', issues_viewCount);
		} else if (metadata !== null) {
			var issues_viewMetadata = issues_viewDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(issues_viewMetadata, null, 2));
		} else {
			var issues_view = issues_viewDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(issues_view, null, 2));
		}
	}
}
