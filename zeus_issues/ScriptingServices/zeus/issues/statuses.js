...
var statusesDao = require('zeus/issues/statusesDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(null, count, metadata, httpResponse)) {
		if (count !== null) {
			var statusesCount = statusesDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', statusesCount);
		} else if (metadata !== null) {
			var statusesMetadata = statusesDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(statusesMetadata, null, 2));
		} else {
			var statuses = statusesDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(statuses, null, 2));
		}
	}
}
