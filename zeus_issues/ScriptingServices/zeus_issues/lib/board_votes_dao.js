...
var BoardVotesORM = {
	dbName: "ZEUS_ISSUES_BOARD_VOTE",
	properties: [
		{
			name: "id",
			dbName: "ZEUS_ISSUESV_ID",
			id: true,
			required: true,
			type: "Long"
		},{
			name: "boardId",
			dbName: "ZEUS_ISSUESV_ZEUS_ISSUESB_ID",
			required: true,
			type: "Long"
		},{
			name: "user",
			dbName: "ZEUS_ISSUESV_USER",
			required: true,
			type: "String",
			size: 100
		},{
			name: "vote",
			dbName: "ZEUS_ISSUESV_VOTE",
			required: true,
			type: "Short"
		}
	]
...
BoardVotesDAO.prototype.vote = function(boardId, user, vote){
	...
	if(previousVote === undefined || previousVote === null || previousVote === 0){
		...
		this.$log.info("Inserting "+this.orm.dbName+" relation between ZEUS_ISSUES_BOARD["+boardId+"] and IAM_USER["+user+"]");
		this.insert({
			vote: vote,
			boardId: boardId,
			user: user
